...
export const create = asyncHandler(async (req, res) => {
  ...
  await audit(req, { action: 'jobs.create', resource: 'jobs', resourceId: job.id, newValue: { id: job.id, title: job.title } });
  return created(res, job, job.status === 'PUBLISHED' ? 'Job published' : 'Job submitted for review');
});
...
export const update = asyncHandler(async (req, res) => {
  ...
  await audit(req, { action: 'jobs.edit', resource: 'jobs', resourceId: job.id, newValue: { id: job.id, title: job.title } });
