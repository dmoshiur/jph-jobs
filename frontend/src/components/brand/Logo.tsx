...
export function Logo({ compact = false, light = false }: { compact?: boolean; light?: boolean }) {
  // Bdjobs.com uses blue color scheme
    ...
    <Link href="/" className="bdj-logo" aria-label="jobhub - Largest Job Site in Bangladesh">
      <span className="bdj-logo-mark" aria-hidden>
        <svg width="28" height="28" viewBox="0 0 32 32" fill="none">
          <rect width="32" height="32" rx="8" fill={light ? '#1488b8' : '#0066cc'} />
          ...
          <circle cx="16" cy="12.2" r="3.1" fill={light ? '#1488b8' : '#0066cc'} />
        </svg>
      </span>
      <span className="bdj-wordmark" style={{ color }}>
        job<span>hub</span>
      </span>
      {!compact && <span className="sr-only">jobhub - Largest Job Site in Bangladesh</span>}
    </Link>
  );
}
